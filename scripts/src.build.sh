...
GOOS=linux GOARCH=amd64 go build -mod=vendor -trimpath -ldflags="${LD_FLAGS}" -a -tags 'remote netgo osusergo containers_image_openpgp containers_image_fulcio_stub containers_image_rekor_stub' -o "${BDIR}/bin/linux/mint-e2e"
popd
